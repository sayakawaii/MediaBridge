...
- **Transcoding is not part of submission, and is checked a run later.** `createDouga` hands back an id for a video AcFun cannot decode just as readily as for one it can, so every video submission is recorded with `"verification": "pending"` and the *next* run asks what became of it. The wait is deferred rather than inline because `sourceStatus` 2 means both "queued" and "failed", and how long a video sits queued depends on its size: half an hour of 1080p60 stays there far longer than the four minutes a run can afford, so an in-run verdict condemns good videos. A day later there is no ambiguity. A submission confirmed dead is named in the run summary for deletion by hand and its dedup key is released so the item is tried again; one AcFun refused outright — 已退回, 时长超限 — keeps its key, because re-uploading the same file would only repeat it. Anything still unresolved after a week is left alone, so the pending list cannot grow without bound.
